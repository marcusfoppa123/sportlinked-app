...
export const searchAthletesWithFilters = async (filters: AthleteFilters) => {
  try {
...
export const checkIsScout = async (userId: string) => {
  try {
    // @ts-ignore - Function will exist after migration
    const { data, error } = await supabase.rpc('is_scout', {
      user_id: userId
    });
